...
**Trooth witnesses and dates facts. It does not score, rate, rank or certify anyone.** That is a product boundary, not a missing feature, and it decides the fate of a whole class of contribution before it is written. A change that adds a number summarising a company, a grade, a ranking, a pass mark, a badge that means "approved", or a check of a company against a named standard or certification, will be declined however good the code is. Where two sources disagree, both are published and neither wins. Where a fact is unknown, it is reported unknown rather than filled in.
...
Clear, imperative commit messages. One concern per pull request; small, reviewable diffs merge fast.
...
- Some carry code and a test suite with CI on it. Clone, install, run the tests, and make sure they pass before you push.
- Some carry no code at all. `trooth-mcp` is a licence, a README and the manifest published to the MCP Registry; the server it describes runs elsewhere. `trust-verifier-sdk` is a licence, a contributing guide and a security policy today. There is nothing to run in either, and a pull request against one of them is a documentation change.
...
By contributing, you agree your contribution is licensed under the repository's own licence. That is Apache License 2.0 in every public Trooth repository except `trooth-mcp`, whose `LICENSE` file is MIT. Check the `LICENSE` in the repository you are opening the pull request against; it governs, and this sentence does not.
